...
test("updateShortLinkLastAccessTime with base URL", async () => {
    ...
    const initialResult = await db.prepare("SELECT last_accessed_at FROM sl_links_map WHERE short_id = ? AND base_url_id = ?")
        .bind(shortId, baseUrlId)
        .first<{ last_accessed_at: string }>();

    await backend.updateShortLinkLastAccessTime(shortId, baseUrlId, Date.now() + 100000);

    const updatedResult = await db.prepare("SELECT last_accessed_at FROM sl_links_map WHERE short_id = ? AND base_url_id = ?")
        .bind(shortId, baseUrlId)
        .first<{ last_accessed_at: string }>();
...
test("cleanUnusedLinks returns baseUrlId", async () => {
    ...
    await db.prepare("INSERT INTO sl_links_map (short_id, target_url, base_url_id, last_accessed_at) VALUES (?, ?, ?, ?)")
        .bind(shortId, targetUrl, baseUrlId, "1970-01-01 00:00:00")
        .run();
