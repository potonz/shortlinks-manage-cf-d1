...
test("updateShortLinkLastAccessTime with base URL", async () => {
    ...
    const [initialRows] = await connection.execute<ILastAccessedRow[]>(
        "SELECT last_accessed_at FROM sl_links_map WHERE short_id = ? AND base_url_id = ?",
        [shortId, baseUrlId],
    );
...
test("cleanUnusedLinks returns baseUrlId", async () => {
    ...
    await connection.execute(
        "INSERT INTO sl_links_map (short_id, target_url, base_url_id, last_accessed_at) VALUES (?, ?, ?, ?)",
        [shortId, targetUrl, baseUrlId, "1970-01-01 00:00:01"],
    );
